...
- **Severidad:** M · **Probabilidad:** A (medido) · **Estado:** **cerrado 2026-08-29** (`Settings.yolo_input='unfiltered'`, tile `yolo_input` uint8 desde el σ⁰ sin filtrar; commit e2c8de1). Re-validacion: YOLO en pipeline Pd 0.093 → **0.143**, FAR 0.0027 → 0.0040, F1 0.108 → 0.137 — identico al detector aislado (2 191 vs 2 178 predicciones), es decir, el pipeline ya no le cuesta recall a YOLO. Trigger de cierre cumplido (Pd_pipeline ≥ 0.95 × Pd_detector). **Correccion 2026-08-29 (tarde):** en produccion el cambio no fue efectivo hasta `703c9dd`: `PipelineEngine._run_detection` reconstruia los dicts de tesela sin copiar `yolo_input`, y el fallback disenado para ese caso lo dejo escrito en `execution_log.notes` (`yolo_input:fallback_filtered=1334/1334` en las ejecuciones 58c7afdd/9a5cf2ce de la terna de las 09:49Z). La terna por bandas (65c644b3…) es la primera de produccion con YOLO sobre la tesela sin filtrar: fused 161 → 187, yolo 15 → 63 sobre la misma imagen. Test de regresion en `tests/test_pipeline/test_engine_band_streaming.py`.
...
- **Severidad:** A · **Probabilidad:** A (medido) · **Estado:** **mitigado en repo 2026-08-29** — `Settings.tile_stream_band_rows=4`: `PreprocessStream` (src/pipeline/preprocessing.py) produce las teselas por bandas de 4 filas y `PipelineEngine._detect_bands` las consume bajo un unico run de perfil (memory guard y throttle compartidos), thumbnails por banda y dedup global al final; `preprocess_full` queda como envoltorio (misma salida tesela a tesela, test de equivalencia). **Verificado en OCI 2026-08-29** (misma imagen, FP32 e INT8): RSS pico 4.9 → 2.7 GB, `sat-high` pasa de OOM a **success** (INT8 6.3 min, FP32 51.9 min, 505 / 533 detecciones, identicas a `ground`); `sat-mid`/`sat-low`/`sat-extreme` siguen en OOM a ~2.66–2.72 GB = suelo del proceso (PyTorch + ORT + GDAL + 1 banda), independiente del modelo. **Residual abierto:** bajar el suelo (banda de 1 fila ≈ −0.3 GB; inferencia ONNX sin importar PyTorch; medir RSS base del contenedor) para que `sat-mid` (2 GB) complete (registrado 2026-08-29)
